...
public class MultiImageSelector {
    ...
    public MultiImageSelector setCrop(boolean isCrop) {
        this.isCrop = isCrop;
        return sSelector;
    }

    public MultiImageSelector setCropWight(int cropWight) {
        this.cropWight = cropWight;
        return sSelector;
    }

    public MultiImageSelector setCropHeight(int cropHeight) {
        this.cropWight = cropWight;
        return sSelector;
    }
    ...
    private Intent createIntent(Context context) {
        ...
        if (mMode == MultiImageSelectorActivity.MODE_SINGLE)
            intent.putExtra(MultiImageSelectorActivity.EXTRA_IS_CROP, isCrop);//是否裁剪当mode为single的时候
        else {
            intent.putExtra(MultiImageSelectorActivity.EXTRA_IS_CROP, false);
        }
        return intent;
    }
}
